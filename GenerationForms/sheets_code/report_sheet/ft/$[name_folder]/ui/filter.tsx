...
interface Params {
    ...
    getData: (filter: $[name_entity_ft]FilterDto) => void;
    buttonFilterRef?: RefObject<HTMLButtonElement> | undefined;
}

interface $[name_entity_ft]FilterForm {
    download: DownloadDataResumeDto | null;
}

export default function $[name_entity_ft]Filter({downloadRecords, getData, buttonFilterRef}: Params) {

    const {control, handleSubmit, formState: { errors }} = useForm<$[name_entity_ft]FilterForm>({
        defaultValues: {
            download: null
        }
    });
    ...
    const onSubmit: SubmitHandler<$[name_entity_ft]FilterForm> = async (value) => {
        const filter: $[name_entity_ft]FilterDto = {
