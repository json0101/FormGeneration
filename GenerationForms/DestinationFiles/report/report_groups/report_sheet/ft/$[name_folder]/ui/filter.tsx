...
interface Params {
    ...
    getData: (filter: GroupReportFilterDto) => void;
    buttonFilterRef?: RefObject<HTMLButtonElement> | undefined;
}

interface GroupReportFilterForm {
    download: DownloadDataResumeDto | null;
}

export default function GroupReportFilter({downloadRecords, getData, buttonFilterRef}: Params) {

    const {control, handleSubmit, formState: { errors }} = useForm<GroupReportFilterForm>({
        defaultValues: {
            download: null
        }
    });
    ...
    const onSubmit: SubmitHandler<GroupReportFilterForm> = async (value) => {
        const filter: GroupReportFilterDto = {
