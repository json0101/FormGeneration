...
interface Params {
    user-roleGrid: UserRoleGridDto[];
}

export default function UserRoleGrid({user-roleGrid}: Params) {
        ...
        //   confirmationMessage('¿Seguro que desea eliminar esta información?', '', async () => {
            startTransition(async () => {
              try {
                const response = await fetch(`/sec/user-role/api/${params?.row.id}`, {
                  method: 'DELETE',
                  headers: {
                ...
                const user-roleFiltered = 
                  user-roleGrid.filter(s => s.id !== params?.row.id)
                          .filter(x => ids?.find( id => id === x.id));

                apiRef?.current?.setRows(user-roleFiltered);
                closeDialog();
              } catch (error) {
        ...
        //   });        
        }, [apiRef, closeDialog, user-roleGrid]);
            ...
            () => [
            {
              ...
              getActions: (params: any) => [
                <GridActionsCellItem
                  ...
                  icon={
                    <Link 
                      href={`/sec/user-role/edit/${params.id}`}
                      style={{
                        textDecoration: "none",
                      }}
                    >
                ...
                <div style={{ height: "85vh", width: '100%' }}>
                    <DataGrid
                      ...
                      onRowDoubleClick={(params: GridRowParams)=> {
                        router.push(`/sec/user-role/edit/${params.id}`);
                      }}
                      autoPageSize
                      getRowId={(row) => row.id}
                    />
