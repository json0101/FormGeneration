...
import UserRoleEditComponent from "./ui/user-role-edit.component";
...
export default async function UserRoleEditPage({params} : {params: Promise<{userRoleId: string}>}) {
    ...
    const user-rolePromise = fetch(`${config_local.backendUserAppUrl}/userRole/to-edit/${userRoleId}`, {
        method: 'GET',
        headers: {
    ...
    return(
        <>
            <Box>
                <h1>Editar Usuario - Rol</h1>
            </Box>
            <Suspense fallback={<Loading/>}>
                <Box sx={{height: '100vh'}}>
                    <UserRoleEditComponent 
                        user-rolePromise={user-rolePromise}
                    />
                </Box>
            </Suspense>
