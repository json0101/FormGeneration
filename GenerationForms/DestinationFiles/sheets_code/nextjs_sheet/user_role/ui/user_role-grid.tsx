...
interface Params {
    user_roleGrid: UserRoleGridDto[];
}

export default function UserRoleGrid({user_roleGrid}: Params) {
        ...
        //   confirmationMessage('¿Seguro que desea eliminar esta información?', '', async () => {
            startTransition(async () => {
              try {
                const response = await fetch(`/sec/user_role/api/${params?.row.id}`, {
                  method: 'DELETE',
                  headers: {
                ...
                const user_roleFiltered = 
                  user_roleGrid.filter(s => s.id !== params?.row.id)
                          .filter(x => ids?.find( id => id === x.id));

                apiRef?.current?.setRows(user_roleFiltered);
                closeDialog();
              } catch (error) {
        ...
        //   });        
        }, [apiRef, closeDialog, user_roleGrid]);
            ...
            () => [
            {
              ...
              getActions: (params: any) => [
                <GridActionsCellItem
                  ...
                  icon={
                    <Link 
                      href={`/sec/user_role/edit/${params.id}`}
                      style={{
                        textDecoration: "none",
                      }}
                    >
                ...
                <div style={{ height: "85vh", width: '100%' }}>
                    <DataGrid
                      ...
                      onRowDoubleClick={(params: GridRowParams)=> {
                        router.push(`/sec/user_role/edit/${params.id}`);
                      }}
                      autoPageSize
                      getRowId={(row) => row.id}
                    />
