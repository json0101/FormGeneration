...
export async function DELETE(req: NextRequest, props: { params: Promise<{ userRoleId: string }>}) {
    const params = await props.params;

    try{
        const token = req.cookies.get('session_bk')?.value;
        
        const res2 = await fetch(`${config_local.backendUserAppUrl}/userRole/${params.userRoleId}`, {
            method: 'DELETE',
            headers: {
